...
if (!skipsalt)
{
    ...
    for (int i = 0; i < 16; ++i) { Server.salt += rndchars[rnd.Next(rndchars.Length)]; }
		...
		public static void SaveProps(StreamWriter w) {
			...
			w.WriteLine();
            w.WriteLine("#   mcforge-protection-level\t\t= Choose between: Dev/Mod/Off (default is Off). When set to Mod, MCForge_Redux Moderators AND Developers are protected. When set to Dev, MCForge_Redux Developers only are protected. When set to Off, MCforge staff are not protected.");
            w.WriteLine();
