...
namespace MCForge
{
    public abstract class RelayBotCmd : Command 
    {
        ...
        public override void Use(Player p, string message) {
            ...
            if (message.CaselessEq("reset") || message.CaselessEq("reconnect")) {
                if (!bot.Enabled) { p.SendMessage("{0} is not enabled", bot.RelayName); }
                bot.Reset();
            } else if (message.CaselessEq("connect")) {
                string err = bot.Connect();
                if (err != null) p.SendMessage("{0} bot {1}", bot.RelayName, err);
        ...
        public override void Help(Player p) {
            ...
            p.SendMessage("&T/{0} connect", cmd);
            p.SendMessage("&HCauses the {0} bot to connect to {0}.", relay);
            p.SendMessage("&T/{0} disconnect", cmd);
