...
namespace Starter
{
    class Program
    {
        ...
        public static string DLLLocation
        {
            get
            {
                return "https://github.com/SuperNova-DeadNova/MCForge-Shared/raw/master/Uploads/MCForge_Redux_.dll";
            }
        }
        static int tries = 0;
        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                if (File.Exists("MCForge_Redux_.dll.backup"))
                    File.Delete("MCForge_Redux_.dll.backup");
            }
            catch { }
            if (tries > 4)
            {
                ...
                Console.WriteLine("If you have any issues, get the files from the " + Server.UploadsURL+ " and try again.");
                Console.WriteLine("Press any key to close me...");
                MessageBox.Show("Unable to download MCForge_Redux_.dll.  Please download it manually at " + DLLLocation + ", place it in the same folder as this executable, and restart this application", "Required DLL Missing", MessageBoxButtons.OK, MessageBoxIcon.Error);
                Console.ReadLine();
                Console.WriteLine("Bye!");
            }
            else if (File.Exists("MCForge_Redux_.dll"))
            {

                //Crash issue fixed by re-executing the exe to properly load MCForge_Redux_.dll.
                if (!needsToRestart)
                    openServer(args);
                else
            ...
            else
            {
                ...
                try
                {
                    ...
                    Client.DownloadFile(DLLLocation, "MCForge_Redux_.dll");
                    Client.Dispose();
                ...
                {
                    tries = 5;
                    Console.WriteLine("\nAn error occured while attempting to download MCForge_Redux_.dll\n");
                    Main(args);
                }
            }
        }
        ...
        {
            MCForge_Redux.Gui.Program.Main(args);
        }
    }
}
